...
import { ChevronLeft, ChevronRight, Package, FolderOpen, Wrench, GraduationCap, TrendingUp, Activity, Box as BoxIcon, MapPin, Clock } from 'lucide-react'
      ...
      <div className="flex-1 overflow-auto space-y-4 pr-2">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            return (
              <div key={i} className="group bg-[#1a1f37] rounded-xl p-6 border border-gray-800 hover:border-blue-500/50 transition-all duration-200 cursor-pointer hover:-translate-y-1 hover:shadow-2xl hover:shadow-blue-500/20 relative overflow-hidden">
                <div className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br ${item.color} opacity-10 rounded-full blur-2xl group-hover:opacity-20 transition-opacity`}></div>
        ...
        <div className="bg-[#1a1f37] rounded-xl border border-gray-800 p-6 shadow-xl">
          <div className="flex items-start gap-4 mb-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-1">
                ...
                <div className="px-2 py-0.5 bg-green-500 text-white text-[10px] font-bold rounded uppercase tracking-wide shadow-lg shadow-green-500/50">Live</div>
              </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {['How do I calibrate G05?', 'Show maintenance schedule', 'Find training videos'].map((q, i) => (
              <button key={i} className="text-left px-4 py-3 bg-[#0f1425] hover:bg-[#1e2442] rounded-lg text-sm text-gray-400 transition-all duration-200 border border-transparent hover:border-blue-500/30 hover:text-gray-300 hover:shadow-lg">
                {q}
              </button>
            ))}
          </div>
